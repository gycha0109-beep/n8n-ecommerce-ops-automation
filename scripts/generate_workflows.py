...
nodes += [node('Attach Duplicate Result','n8n-nodes-base.code',2,-180,-80,{'jsCode':"const p=$('Validate Canonical Order').item.json; return [{json:{...p,is_duplicate:Boolean($json.is_duplicate)}}];"})]
...
def http_attempt(name,x,y):
    return node(name,'n8n-nodes-base.httpRequest',4.3,x,y,{'method':'POST','url':'={{ ($env.MOCK_API_BASE_URL || "http://mock-api:3000") + "/api/warehouse/reserve" }}','sendBody':True,'contentType':'raw','rawContentType':'application/json','body':'={{ JSON.stringify({order_key: $json.order_key, sku: $json.sku, quantity: $json.quantity, mode: $json.integration_mode}) }}','options':{'response':{'response':{'fullResponse':True,'neverError':True}},'timeout':1200}})

def eval_attempt(name,x,y,attempt):
    return node(name,'n8n-nodes-base.code',2,x,y,{'jsCode':f"const p=$('Classify Inventory').item.json; const status=Number($json.statusCode ?? 599); return [{{json:{{...p,http_status:status,attempt:{attempt},api_ok:status>=200&&status<300,api_permanent:status>=400&&status<500}}}}];"})
...
nodes += [node('Mark First Attempt Success','n8n-nodes-base.code',2,1570,-420,{'jsCode':"const p=$('Classify Inventory').item.json; return [{json:{...p,retry_recovered:false}}];"})]
nodes += [node('Mark Retry Recovered 2','n8n-nodes-base.code',2,2670,-260,{'jsCode':"const p=$('Classify Inventory').item.json; return [{json:{...p,retry_recovered:true}}];"})]
nodes += [node('Mark Retry Recovered 3','n8n-nodes-base.code',2,3550,-60,{'jsCode':"const p=$('Classify Inventory').item.json; return [{json:{...p,retry_recovered:true}}];"})]
...
nodes += [node('Attach Commit Result','n8n-nodes-base.code',2,4210,-260,{'jsCode':"const p=$('Classify Inventory').item.json; return [{json:{...p,db_result:$json.result,retry_recovered:Boolean($json.retry_recovered)}}];"})]
nodes += [node('Notify Processed','n8n-nodes-base.httpRequest',4.3,4430,-260,{'method':'POST','url':'={{ $env.ALERT_WEBHOOK_URL || "http://mock-api:3000/api/notify" }}','sendBody':True,'contentType':'raw','rawContentType':'application/json','body':'={{ JSON.stringify({event:"order_processed",order_key:$json.order_key,mode:$json.notification_mode}) }}','options':{'response':{'response':{'fullResponse':True,'neverError':True}},'timeout':2000}})]
...
nodes += [node('Record Notification Failure','n8n-nodes-base.postgres',2.5,4870,-80,{'operation':'executeQuery','query':"SELECT record_exception_b64($1, 'NOTIFICATION_FAILURE', 'Order was stored but downstream notification failed');",'options':{'queryReplacement':"={{ $('Classify Inventory').item.json.payload_b64 }}"}})]
nodes += [node('Respond Success','n8n-nodes-base.respondToWebhook',1.4,4870,-360,{'respondWith':'json','responseBody':'={{ {ok:true,order_key:$("Classify Inventory").item.json.order_key,status:"processed"} }}','options':{'responseCode':200}})]
nodes += [node('Respond Exception','n8n-nodes-base.respondToWebhook',1.4,620,300,{'respondWith':'json','responseBody':'={{ {ok:false,order_key:$("Validate Canonical Order").item.json.order_key,status:"exception"} }}','options':{'responseCode':202}})]
nodes += [node('Respond Failure','n8n-nodes-base.respondToWebhook',1.4,3990,180,{'respondWith':'json','responseBody':'={{ {ok:false,order_key:$("Classify Inventory").item.json.order_key,status:"dead_letter"} }}','options':{'responseCode':202}})]
...
main={'name':'E-commerce Order Intake — Reliability Demo','nodes':nodes,'pinData':{},'connections':C,'active':False,'settings':{'executionOrder':'v1','errorWorkflow':''},'versionId':uid('main-version'),'meta':{'templateCredsSetupCompleted':False},'tags':[]}
...
write('ecommerce-error-handler.json',{'name':'E-commerce Ops — Global Error Handler','nodes':errnodes,'pinData':{},'connections':errcon,'active':False,'settings':{'executionOrder':'v1'},'versionId':uid('err-version'),'meta':{'templateCredsSetupCompleted':False},'tags':[]})
...
sumnodes=[
 ...
 node('Send Summary','n8n-nodes-base.httpRequest',4.3,180,0,{'method':'POST','url':'={{ $env.ALERT_WEBHOOK_URL || "http://mock-api:3000/api/notify" }}','sendBody':True,'contentType':'raw','rawContentType':'application/json','body':'={{ JSON.stringify($json) }}','options':{'response':{'response':{'fullResponse':True,'neverError':True}},'timeout':2000}})
]
...
write('ecommerce-daily-summary.json',{'name':'E-commerce Ops — Daily Summary','nodes':sumnodes,'pinData':{},'connections':sumcon,'active':False,'settings':{'executionOrder':'v1','timezone':'Asia/Seoul'},'versionId':uid('sum-version'),'meta':{'templateCredsSetupCompleted':False},'tags':[]})
