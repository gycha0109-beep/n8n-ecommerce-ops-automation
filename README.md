...
- n8n **2.37.7 stable** reference version
- PostgreSQL 16 / Supabase-compatible SQL
- Node.js 22 mock API
...
## Current verified synthetic result

Generated by the local contract smoke harness from the repository fixtures:
...
All Tests **A–I passed** in the current artifact build. The harness also replays the complete dataset against a cloned database and verifies that order count and inventory remain unchanged.

### Verification scope

- **Executed here:** mock API runtime, all 42 fixtures, validation/idempotency/inventory/retry/dead-letter logic, notification failure, re-run invariants, DB-derived summary, workflow JSON structural/connection/security checks.
- **Deployment target:** PostgreSQL/Supabase + n8n 2.37.7.
- **Environment-dependent:** a real n8n container import/execution and PostgreSQL container run require Docker on the machine running the demo.

`generated/smoke-report.json` explicitly records that the deterministic local contract harness uses SQLite to mirror the PostgreSQL logical contract. It is not presented as proof of a live Supabase deployment.
